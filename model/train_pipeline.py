...
def run_training() -> None:
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        data[config.local_model_config.features],  # predictors
        data[config.local_model_config.target],
        test_size=config.local_model_config.test_size,
        # we are setting the random seed here
        # for reproducibility
        random_state=config.local_model_config.random_state,
    )
    #y_train = np.log(y_train)
    y_train = y_train.map(config.local_model_config.qual_mappings)
